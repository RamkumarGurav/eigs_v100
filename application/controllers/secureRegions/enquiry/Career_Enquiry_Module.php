...
class Career_Enquiry_Module extends Main
{
	...
	function __construct()
	{
		...
		$this->load->model('administrator/enquiry/Career_Enquiry_Model');
	...
	function index()
	{
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/enquiry/career_enequiry/listings', $this->data);
		parent::get_footer();
	}
	...
	function listings()
	{
		...
		$data_count = $this->Career_Enquiry_Model->get_career_enquiry($search);
		...
		$this->data['career_enquiry_data'] = $this->Career_Enquiry_Model->get_career_enquiry($search);

		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/enquiry/Career_Enquiry_Module/listings', $this->data);
		parent::get_footer();
	}

	function view($career_enquiry_id = "")
	{
		...
		$this->data['career_enquiry_data'] = $this->Career_Enquiry_Model->get_career_enquiry(array("career_enquiry_id" => $career_enquiry_id, "details" => 1));
		...
		$this->data['career_enquiry_data'] = $this->data['career_enquiry_data'][0];

		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/enquiry/Career_Enquiry_Module/view', $this->data);
		parent::get_footer();
	}
	...
	function edit($career_enquiry_id = "")
	{
		$this->data['page_type'] = "list";
		$user_access = $this->data['user_access'] = $this->data['User_auth_obj']->check_user_access(array("module_id" => $this->data['page_module_id']));
		//print_r($this->data['user_access']);
		...
		if (!empty($career_enquiry_id)) {
			$this->data['career_enquiry_data'] = $this->Career_Enquiry_Model->get_career_enquiry(array("career_enquiry_id" => $career_enquiry_id, "details" => 1));
			if (empty($this->data['career_enquiry_data'])) {
	...
	function doEdit()
	{
		...
		$career_enquiry_id = $_POST['career_enquiry_id'];
		...
		if (!empty($career_enquiry_id)) {
			...
			$insertStatus = $this->Common_Model->update_operation(array('table' => 'enquiry', 'data' => $enter_data, 'condition' => "career_enquiry_id = $career_enquiry_id"));
			...
			$career_enquiry_id = $insertStatus = $this->Common_Model->add_operation(array('table' => 'enquiry', 'data' => $enter_data));
			if (!empty($insertStatus)) {
	...
	function doUpdateStatus()
	{
		...
		if ($user_access->update_module == 1) {
			...
			if (!empty($id_arr)) {
				...
				$response = $this->Common_Model->update_operation(array('table' => "enquiry", 'data' => $update_data, 'condition' => "career_enquiry_id in ($ids)"));
				if ($response) {
