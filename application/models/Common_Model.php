...
class Common_Model extends CI_Model
{
	...
	function send_mail($params = array())
	{
		...
		$from_email = "info@marswebsolution.com";
		$from_name = "Mars Web Solutions";

		$request = $error_info = "";
		$GATEWAYAPI = 'http://enterprise.webaroo.com/GatewayAPI/rest';
		...
		if (!empty($post_data['bcc']))
			$post_data['bcc'] = $params['bcc'];
		if (!empty($post_data['cc']))
			$post_data['cc'] = $params['cc'];
		...
		// return $entereddatamaillog['status'];
	}
	...
	function send_sms($mobile, $template)
	{
		...
		$url = "http://tsms.marswebhosting.com/GatewayAPI/rest?" . $request;
		//	echo $url; echo "</br>";
		$ch = curl_init($url);
	...
	{
		//echo "ANU";
		////$params['to'] = 'anubhav.gupta@marswebsolutions.com';
		$attachment1 = '';
		$template = $params['template'];
		$subject = $params['subject'];
		$to = $params['to'];
		...
		$entereddatamaillog['template'] = $template;
		$entereddatamaillog['to'] = $to;
		$entereddatamaillog['mt_id'] = $mt_id;
		$entereddatamaillog['added_on'] = date('Y-m-d H:i:s');

		$from_email = "info@marswebsoutions.ooo";
		$from_name = "Mars Web Solution";

		$request = $error_info = "";
		$GATEWAYAPI = 'http://enterprise.webaroo.com/GatewayAPI/rest';
		$post_data['method'] = "EMS_POST_CAMPAIGN";
		$post_data['userid'] = "2000701148";
		$post_data['password'] = "MAhDxE";
		$post_data['recipients'] = $to;
		...
		//$fy = new fiscalYear();
		//$getFiscalEmailNumberArr = $fy->getFiscalEmailNumber();
		//$getFiscalEmailNumber = $getFiscalEmailNumberArr['email_number'];
		//$fiscal_year_id = $getFiscalEmailNumberArr['fiscal_year_id'];

		$entereddataemail['email_number'] = $id;

		$this->update_operation(array('table' => 'email_log', 'data' => $entereddataemail, 'condition' => "id = " . $id));
		//echo "<pre>"; print_r($post_data);echo "</pre>";
		return $mail_status;
	}

	function SetMailContent($title, $content)
	{
		$data = '<tr><td style="padding:10px; text-align:left; font-family:Arial, Helvetica, sans-serif; font-size:14px; border-bottom:1px solid #e2e2e2"><strong style="width:120px; display:inline-block;">' . $title . '</strong> : ' . $content . '</td></tr>';
		return $data;
	}
