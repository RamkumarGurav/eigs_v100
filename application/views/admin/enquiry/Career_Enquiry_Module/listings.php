                            ...
                            <table id="example1" class="table table-bordered table-hover table-striped">
                                ...
                                <? if (!empty($career_enquiry_data)) { ?>
                                    <tbody>
                                        ...
                                        foreach ($career_enquiry_data as $urm) {
                                            $count++;
                                            ?>
                                            <tr>
                                                <td><?= $count ?>.</td>
                                                <?php if ($user_access->update_module == 1) { ?>
                                                    <td><input type="checkbox" name="sel_recds[]" id="sel_recds<?php echo $count; ?>"
                                                            value="<?php echo $urm->career_enquiry_id; ?>" /></td>
                                                <? } ?>
                                                <td><a
                                                        href="<?= MAINSITE_Admin . $user_access->class_name . "/view/" . $urm->career_enquiry_id ?>"><?= date("d-m-Y", strtotime($urm->added_on)) ?></a>
                                                </td>
                                                <td><?= $urm->name ?></td>
                                                <td><?= $urm->email ?></td>
                                                <td><?= $urm->contactno ?></td>
                                                <td> <?php if (!empty($urm->resume_file)): ?>
                                                        <a target="_blank" class="btn btn-outline-primary btn-sm"
                                                            href="<?= _uploaded_files_ . 'resume_file/' . $urm->resume_file ?>">
                                                            view
                                                        </a>
                                                    <?php else: ?>
                                                        -
